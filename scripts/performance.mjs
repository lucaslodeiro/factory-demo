...
try{for(const path of ['/','/en/','/pt/']){const runs=[];for(let i=0;i<3;i++){const {lhr,report}=await lighthouse('http://127.0.0.1:4321'+path,{port:chrome.port,output:'json',onlyCategories:['performance'],formFactor:'mobile',logLevel:'error'});await writeFile(`evidence/lighthouse/${path==='/'?'es':path.replaceAll('/','')}-${i+1}.json`,report);runs.push({performance:lhr.categories.performance.score*100,lcp:lhr.audits['largest-contentful-paint'].numericValue,cls:lhr.audits['cumulative-layout-shift'].numericValue,version:lhr.lighthouseVersion});}const result={path,runs,median:{performance:median(runs.map(r=>r.performance)),lcp:median(runs.map(r=>r.lcp)),cls:median(runs.map(r=>r.cls))}};summary.push(result);console.log(result);assert.ok(result.median.performance>=90);assert.ok(result.median.lcp<=2500);assert.ok(result.median.cls<=.1);}
