...
await writeFile('evidence/performance.json',JSON.stringify(summary,null,2));for(const result of summary.filter(r=>r.median)){assert.ok(result.median.performance>=90,`${result.path} Performance`);assert.ok(result.median.lcp<=2500,`${result.path} LCP`);assert.ok(result.median.cls<=.1,`${result.path} CLS`);}}finally{if(!managedPort)await chrome.kill();}
