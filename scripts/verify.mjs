...
for(const [lang,path] of [['es','/'],['en','/en/'],['pt','/pt/']]){
 ...
 const context=await browser.newContext();const page=await context.newPage();await page.goto(base+path);
 ...
 for(const id of ['developers','telcos','apis','platform','contact']){await page.goto(base+path+'#'+id);for(const a of await page.locator('[data-language]').all())assert.ok((await a.getAttribute('href')).endsWith('#'+id));}
 ...
 for(const width of [360,768,1440]){await page.setViewportSize({width,height:900});await page.goto(base+path);await page.evaluate(()=>document.fonts.ready);assert.ok(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth),`overflow ${lang} ${width}`);const axe=await new AxeBuilder({page}).withTags(['wcag2a','wcag2aa','wcag21aa']).analyze();assert.deepEqual(axe.violations,[],JSON.stringify(axe.violations));await page.screenshot({path:`evidence/${lang}-${width}.png`,fullPage:true});report.push({lang,width,axeViolations:axe.violations.length,horizontalOverflow:false});}
 ...
 await page.goto(base+path);const focus=[];for(let i=0;i<24;i++){await page.keyboard.press('Tab');focus.push(await page.locator(':focus').evaluate(e=>({text:e.textContent.trim()||e.getAttribute('aria-label'),outline:getComputedStyle(e).outlineStyle})));}assert.ok(focus.every(f=>f.text&&f.outline==='solid'));report.push({lang,keyboard:focus});
 await context.close();
 const nojs=await browser.newContext({javaScriptEnabled:false,viewport:{width:360,height:900},reducedMotion:'reduce'});const p=await nojs.newPage();await p.goto(base+path);assert.equal(await p.locator('h1').count(),1);await p.locator('a[href="#apis"]').first().click();assert.equal(new URL(p.url()).hash,'#apis');assert.equal(await p.locator('[data-language]').count(),3);assert.ok(await p.locator('a[href^="mailto:"]').count()>2);await p.locator('[data-language][lang="en"]').click();assert.equal(await p.locator('html').getAttribute('lang'),'en');await nojs.close();
}
