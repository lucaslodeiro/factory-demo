...
if(form){
 ...
 window.onTurnstileReady=()=>{widget=window.turnstile!.render('#security',{'sitekey':form.dataset.sitekey,'action':`contact_${form.dataset.intent}`,'language':form.dataset.locale,'size':'flexible',callback:(value:string)=>{token=value;},'expired-callback':()=>{token='';},'error-callback':()=>{token='';status.textContent=copy.spamError;}});};
