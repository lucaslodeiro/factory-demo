...
if(form){
 ...
 form.addEventListener('submit',async event=>{
  ...
  const fallback=document.querySelector<HTMLAnchorElement>('#mail-fallback')!;fallback.hidden=true;fallback.removeAttribute('href');
  ...
  try{
   ...
   if(res.ok&&result.fallback==='mailto'){
    const labels=['name','email','company','message','api'];
    const body=[copy.intent+': '+copy.intentLabel,copy.language+': '+form.dataset.locale,...labels.map(key=>copy[key]+': '+(fields[key]||'-'))].join('\r\n');
    fallback.href='mailto:info@openxpand.com?subject='+encodeURIComponent('OpenXpand | '+copy.intentLabel)+'&body='+encodeURIComponent(body);
    fallback.hidden=false;status.textContent=copy.mailDraft;fallback.click();
   }
   else if(res.ok&&result.ok===true){status.textContent=copy.success;form.reset();requestId=crypto.randomUUID();priorPayload='';}
